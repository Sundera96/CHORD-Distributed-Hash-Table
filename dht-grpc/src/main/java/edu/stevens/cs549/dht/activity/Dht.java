...
public class Dht extends DhtBase implements IDhtService, IDhtNode, IDhtBackground {
		...
		if (!pred .hasNodeInfo() || inInterval(cand.getId(), pred.getNodeInfo().getId(), info.getId(), false)) {
			...
			synchronized (state) {
				...
				for(int i=0;i<db.getBindingsCount();i++) {
					state.getBroadcaster().broadcastMovedBinding(db.getBindings(i).getKey());
				}
			}
	...
	protected String[] get(NodeInfo n, String k) throws Failed {
		if (isEqual(n, getNodeInfo())) {
			...
			String[] strArr = new String[bindings.getValueCount()];
			for(int i=0;i<strArr.length;i++){
				strArr[i]=bindings.getValue(i);
			}
			return strArr;
		}
	}
	...
	public void join(String host, int port) throws Failed, Invalid {
		 ...
		 */
		state.clear();
		NodeInfo node = NodeInfo.newBuilder().setId(info.getId()).setHost(host).setPort(port).build();
		succ= client.findSuccessor(node,info.getId());
		setSucc(succ);
		stabilize();
	}
